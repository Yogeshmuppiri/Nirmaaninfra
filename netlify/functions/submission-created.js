...
const buildOwnerEmail = ({ logoUrl, submission }) =>
    buildEmailShell({
        ...
        `,
        footerNote: "This email was generated automatically from the Netlify form submission workflow."
    });
...
const extractSubmission = (eventBody) => {
    const payload = eventBody?.payload || {};
    const data = payload.data || payload;

    return {
        formName: data["form-name"] || payload.form_name || payload.formName || "",
        name: (data.name || "").trim(),
        email: (data.email || "").trim(),
        subject: (data.subject || "Website Inquiry").trim(),
...
exports.handler = async (event) => {
    try {
        ...
        if (submission.formName && submission.formName !== "contact") {
            return {
                statusCode: 200,
                body: JSON.stringify({ skipped: true, reason: "Non-contact form submission." })
            };
        }
        ...
        if (!submission.email || !submission.message) {
            return {
                statusCode: 200,
                body: JSON.stringify({ skipped: true, reason: "Submission missing required fields." })
            };
        }

        const siteUrl = normalizeUrl(parsedBody?.site?.url || process.env.URL);
